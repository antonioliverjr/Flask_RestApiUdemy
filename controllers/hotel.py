...
from views.hotel import HotelModelViewDto
...
class HotelController(Resource):
    def get(self, id:int = None):
        ...
        if id is not None:
            ...
            if result is None:
                ...
                return marshal(result, HotelModelViewDto.message()), 404
            return marshal(result, HotelModelViewDto.response()), 200
        
        result = hotelRepository.list()
        if len(result) == 0:
            return marshal(result, HotelModelViewDto.response()), 404
        return marshal(result, HotelModelViewDto.response()), 200
    
    def post(self):
        data = HotelModelViewDto.request()
        hotelRepository = HotelRepository()
        cityRepository = CityRepository()
        city_obj = cityRepository.search(data.city)
        if city_obj is None:
            result = {'message': f'A Cidade {data.city} informada não encontra-se cadastrada.'}
            return marshal(result, HotelModelViewDto.message()), 400
        try:
            result = hotelRepository.create(data.name, data.stars, data.daily, city_obj)
        except Exception as ex:
            result = {'message': str(ex)}
            return marshal(result, HotelModelViewDto.message()), 400
        return marshal(result, HotelModelViewDto.response()), 201
    ...
    def delete(self, id:int):
        ...
        if hotelRepository.list_id(id):
            ...
            return marshal(result, HotelModelViewDto.message()), 200
        result = {'message': f'Id não localizado, verifique Id:{id} foi o informado.'}
        return marshal(result, HotelModelViewDto.message()), 404
